(function($) {
  ...
  function init(plot) {
    ...
    function segmentIntersection(
      ...
      var top_delta_x, top_delta_y, bottom_delta_x, bottom_delta_y, s, t;
      ...
      t =
        (bottom_delta_x * (top_left_y - bottom_left_y) -
          bottom_delta_y * (top_left_x - bottom_left_x)) /
        (-bottom_delta_x * top_delta_y + top_delta_x * bottom_delta_y);
    ...
    function plotDifferenceArea(plot, ctx, series) {
      ...
      var otherseries, ps, points, otherps, otherpoints, plotOffset, fillStyle;

      function openPolygon(x, y) {
        ctx.beginPath();
        ctx.moveTo(series.xaxis.p2c(x) + plotOffset.left, series.yaxis.p2c(y) + plotOffset.top);
      }
      ...
      function findNextStart(start_i, end_i) {
        ...
        var start =
            start_i === 0 || points[start_i - 1] === null || otherpoints[start_i - 1] === null,
          equal = false,
          i,
          intersect;
        ...
        for (i = start_i; i < end_i; i++) {
          ...
          if (points[i * ps + 1] === null || otherpoints[i * ps + 1] === null) {
            equal = false;
            start = true;
          }
      ...
      function intersectionPoint(right_i) {
        ...
        for (i = 1; i < otherpoints.length / otherps; i++) {
          intersect = segmentIntersection(
            points[(right_i - 1) * ps],
            points[(right_i - 1) * ps + 1],
            points[right_i * ps],
            points[right_i * ps + 1],

            otherpoints[(i - 1) * otherps],
            otherpoints[(i - 1) * otherps + 1],
            otherpoints[i * otherps],
            otherpoints[i * otherps + 1]
          );
